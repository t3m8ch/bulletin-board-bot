...
async def cmd_browse_ads(message: types.Message, user_data):
...
async def cq_next_ad_handler(call: types.CallbackQuery, callback_data: dict, user_data):
...
async def cq_back_ad_handler(call: types.CallbackQuery, callback_data: dict, user_data):
    user_id = call.from_user.id
    ad_service = user_data[user_id]["ad_service"]
